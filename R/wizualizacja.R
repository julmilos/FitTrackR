...
.motyw_mcda <- function() {
  list(
    ...
    theme(
      plot.title = element_text(face = "bold", size = 16, color = "#263238"),
      plot.subtitle = element_text(color = "#607D8B", size = 11),

      panel.border = element_rect(color = "#B0BEC5", fill = NA, linewidth = 0.5),

      legend.position = "right",

      axis.title = element_text(face = "bold", color = "#263238"),
...
plot.rozmyty_vikor_wynik <- function(x, ...) {
  ...
  ggplot(df, aes(x = Wydajnosc, y = Def_R)) +
    ...
    annotate(
      "rect",
      xmin = srodek_perf, xmax = Inf,
      ymin = -Inf, ymax = srodek_ryzyko,
      fill = "#E8F5E9", alpha = 0.6
    ) +

    geom_vline(xintercept = srodek_perf, linetype = "dashed", color = "#90A4AE") +
    geom_hline(yintercept = srodek_ryzyko, linetype = "dashed", color = "#90A4AE") +
    ...
    geom_point(
      aes(size = Rozmiar, fill = Wydajnosc),
      shape = 21,
      color = "#1B5E20",
      alpha = 0.85
    ) +
